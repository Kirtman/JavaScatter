...
var scatter = function(){
	...
	d3.csv("js/AllOccupations.csv", function(data) {
		...
		var xMax = d3.max(data, function(d) { return +d.TotalEmployed2011; }) * 1,
			xMin = 0,
			yMax = d3.max(data, function(d) { return +d.MedianSalary2011; }) * 1,
			yMin = 0,
...
//			yMed = d3.median(data, function(d) { return +d.MedianSalary2011;}),
		...
		var colourScale = function(val,array,active){
			if (val > 30) {
				return array[0]; //Enemy
			} else if (val > 10) {
				return array[1]; //Ally
			} else if (val > 0) {
				return array[2]; //Your Cities
			} else {
